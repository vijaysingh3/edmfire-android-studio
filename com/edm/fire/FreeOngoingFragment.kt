...
class FreeOngoingFragment : Fragment() {
    ...
    private val TAG = "FREE_DEBUG_ONGOING"
    ...
    private fun loadOngoingTournaments() {
        if (currentDatabaseUrl.isEmpty()) {
            showError("Database URL missing")
            return
        }

        showLoading(true)
        val url = "$currentDatabaseUrl/Tournaments/TournamentMeta/FreeTournaments.json?orderBy=\"Status\"&equalTo=\"Ongoing\""
        ...
        try {
            ...
            for ((key, value) in jsonObject.entrySet()) {
                ...
                val tournament = Tournament(
                    ...
                    PricePool = getInt(obj, "PricePool"), // ⚡ FIX: PrizePool -> PricePool
    ...
    private fun updateUI() {
        if (tournamentList.isEmpty()) {
            tvEmptyMessage.text = "No ongoing free tournaments available"
    ...
    companion object {
        fun newInstance(databaseUrl: String): FreeOngoingFragment {
            return FreeOngoingFragment().apply {
