...
class OnGoingActivity : AppCompatActivity() {
    ...
    private val ongoingMatchesList = mutableListOf<OnGoingMatch>()
    private lateinit var adapter: OnGoingMatchAdapter
    ...
    private fun setupToolbar() {
        findViewById<com.google.android.material.appbar.MaterialToolbar>(R.id.toolbar).apply {
            setNavigationOnClickListener { onBackPressed() }
            title = "My OnGoing Matches"
        }
    }

    private fun setupRecyclerView() {
        adapter = OnGoingMatchAdapter(ongoingMatchesList)
    ...
    private fun loadUserOnGoingMatches() {
     ...
     *
     * Fields: Title, DateTime, JoiningFee, PrizePool, PerKill, SlotNumbers,
     *         JoinedPlayersCount, BannerUrl, Map, Mode, Type, Status,
     *         RoomID, RoomPassword — SAB ek jagah!
     *
     * NOTE: VideoUrl Meta mein nahi hai — woh TournamentDetails mein hai
     *       Ongoing matches ke liye VideoUrl empty rhega, MatchDetailActivity khud fetch karega
     */
        ...
        UniversalReader.readJson(
            ...
            onResult = { jsonString ->
                try {
                    ...
                    // FILTER: Sirf "Ongoing" status dikhayenge
                    // "OnGoing" aur "Ongoing" dono variations handle karenge
                    if (!status.equals("OnGoing", ignoreCase = true) &&
                    ...
                    val entryFee = json.optInt("JoiningFee", 0)
                    ...
                    // Ongoing matches me room ID/password HONE CHAHIYE (match chal raha hai)
                    ...
                    // MatchDetailActivity khud Details se fetch karega jab user click karega
                    val videoUrl = ""

                    val ongoingMatch = OnGoingMatch(
                        tournamentType = tournamentType,
                        tournamentId = tournamentId,
                        title = title,
                        dateTime = dateTime,
                    ...
                    ongoingMatchesList.add(ongoingMatch)
                } catch (e: Exception) {
