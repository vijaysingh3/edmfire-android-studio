...
class FreePastMatchesFragment : Fragment() {
    ...
    private val TAG = "FREE_DEBUG_PAST"
    ...
    private fun loadPastTournaments() {
        if (currentDatabaseUrl.isEmpty()) {
            showError("URL missing")
            return
        }

        showLoading(true)
        val url = "$currentDatabaseUrl/Tournaments/TournamentMeta/FreeTournaments.json?orderBy=\"Status\"&equalTo=\"Completed\"&limitToLast=2"
    ...
    private fun updateUI() {
        if (tournamentList.isEmpty()) {
            tvEmptyMessage.text = "No past free tournaments available"
    ...
    companion object {
        fun newInstance(databaseUrl: String): FreePastMatchesFragment {
            return FreePastMatchesFragment().apply {
