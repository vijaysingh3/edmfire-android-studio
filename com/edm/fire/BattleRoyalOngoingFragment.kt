...
class BattleRoyalOngoingFragment : Fragment() {
    ...
    private val countRunnables = mutableMapOf<String, Runnable>()
    ...
    override fun onCreateView(
        ...
        val view = inflater.inflate(R.layout.fragment_battle_royal_ongoing, container, false)
    ...
    private fun loadOngoingTournaments() {
        ...
        val url = "$currentDatabaseUrl/Tournaments/TournamentMeta/BattleRoyal.json?orderBy=\"Status\"&equalTo=\"Ongoing\""
    ...
    private fun startRealtimeCountListener(tournamentId: String, index: Int) {
        val url = "$currentDatabaseUrl/Tournaments/TournamentMeta/BattleRoyal/$tournamentId/JoinedPlayersCount.json"
        val runnable = createCountRunnable(url, tournamentId, index)
        countRunnables[tournamentId] = runnable
    ...
    private fun stopAllCountListeners() {
        countRunnables.values.forEach { handler.removeCallbacks(it) }
        countRunnables.clear()
    }
    ...
    private fun updateUI() {
        if (tournamentList.isEmpty()) {
            tvEmptyMessage.text = "No ongoing tournaments available"
    ...
    companion object {
        fun newInstance(databaseUrl: String): BattleRoyalOngoingFragment {
            return BattleRoyalOngoingFragment().apply {
